...
function Seller(props) {
  ...
  const Submit =(data)=>{
    Register(data.email,data.password).then(res=>{
      ...
      const user = {email:res.user.email, name:res.user.displayName, photo:res.user.photoURL ,uid:res.user.uid,role:"seller"}
...
<label className="block">
  ...
  <input  {...register('email')} type="email"  className="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md text-sm shadow-sm placeholder-slate-400
    focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-green-500
